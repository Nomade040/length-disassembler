...
size_t ldisasm(const void* const address)
{
	...
	if (*b == 0x0F) // 2,3 bytes
	{
		...
		else // 2 bytes
		{
			if (LDISM_R == 8) //disp32
				offset += 4;
			else if ((LDISM_R == 7 && LDISM_C < 4) || *b == 0xA4 || *b == 0xC2 || (*b > 0xC3 && *b <= 0xC6) || *b == 0xBA) //imm8
				offset++;
	...
	else // 1 byte
	{
		...
		if ((LDISM_R == 0xE && LDISM_C < 8) || (LDISM_R == 0xB && LDISM_C < 8) || LDISM_R == 7 || (LDISM_R < 4 && (LDISM_C == 4 || LDISM_C == 0xC)) || (*b == 0xF6 && !(*(b + 1) & 56)) || find(op1imm8, *b)) //imm8
			...
			offset += 3;
		else if ((LDISM_R < 4 && (LDISM_C == 5 || LDISM_C == 0xD)) || (LDISM_R == 0xA && LDISM_C < 4) || (LDISM_R == 0xB && LDISM_C >= 8) || (*b == 0xF7 && !(*(b + 1) & 56)) || find(op1imm32, *b)) //imm32,16
